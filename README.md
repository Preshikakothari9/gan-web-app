...
| `API_KEY` | *(unset)* | Shared-secret gate for the training endpoints only (`/api/*/train`, `/api/*/stop`). Unset = those routes stay open, same as before. See "Auth" below. |

## Auth

Training is the only expensive/abusable action this app exposes (it spins
up a background thread and burns CPU for however many epochs you ask for),
so it's the only thing gated. Set `API_KEY` in the environment before
deploying publicly:

```bash
export API_KEY="something-long-and-random"
```

Once set, `POST /api/image/train`, `/api/image/stop`, `/api/text/train`,
and `/api/text/stop` require an `X-API-Key` header matching that value, or
they return `401`. Everything else (`/api/health`, `/api/models`,
`/api/*/status`, `/api/*/generate`) stays open, since those are read-only
and cheap.

The frontend has a small **API key** field in the top-right of the header —
paste the same value there and click Save; it's stored in the browser's
localStorage and sent automatically on train/stop requests. If `API_KEY`
isn't set on the server, you can leave this blank entirely (that's the
default local/demo setup).

This is a lightweight shared-secret, not a full auth system — good enough
to stop a random visitor from hammering "train" on your public demo, not
...
- **`tests/test_api.py`** — hits the Flask app through its test client (no
  real server, no network): health/models/generate endpoints, the JSON-404
  fallback, and the `API_KEY` auth gate (open by default, `401` without the
  key once `API_KEY` is set, `401` with a wrong key, `200` with the right
  one). Training is never actually run in tests — `threading.Thread` is
  monkeypatched so `/train` is exercised without downloading MNIST or
  spinning up a real training loop.
- **`tests/test_models.py`** — unit tests directly on the PyTorch model
  classes: output shapes for the image/text generator and discriminator,
  that generated images stay in `tanh` range and discriminator scores stay
  in `[0,1]`, and — the part most worth testing given how the text GAN
  works — that gradients actually flow from the discriminator back through
  `sample_gumbel` into the generator's weights (the whole point of using
  Gumbel-Softmax + straight-through estimation here).

No GPU or dataset download is required to run the suite.
...
10. **`--extra-index-url` didn't reliably select the CPU-only torch build**
    — `torch==2.9.0` exists on both PyPI (full CUDA build, ~4 GB of
    `nvidia-*-cu12` deps) and the PyTorch CPU index with the *same version
    string*, and pip doesn't reliably prefer one index over the other in
    that situation — in testing it resolved to the GPU build anyway,
    exhausting disk on a free-tier host. Pinned the explicit `+cpu` suffix
    (`torch==2.9.0+cpu`) so pip can only resolve it against the CPU index.
11. **No tests** — added a `pytest` suite (`backend/tests/`) covering the
    API endpoints (including the new auth gate) and the GAN model classes
    (output shapes, value ranges, and gradient flow through Gumbel-Softmax).
    See "Testing" below.
12. **Training endpoints had no auth** — anyone with network access to a
    deployed instance could trigger training runs. Added an optional
    `API_KEY` shared-secret gate on the training/stop endpoints only (see
    "Auth" below); read-only endpoints stay open.

## Known limitations

- **Why GANs, not diffusion?** GANs are lighter-weight and single-pass
  (one forward call through the generator, no iterative denoising), which
  fits a live in-browser demo that trains from scratch in a background
  thread on a free-tier CPU host. A diffusion model would need dozens of
  denoising steps per sample and is a much heavier training/inference
  budget for the same demo. That tradeoff is the honest reason for the
  choice — GANs are also notoriously less stable to train and more prone
  to mode collapse than diffusion, which is why diffusion has become the
  default for production-grade generation.
- The text GAN's vocabulary and "real" training sentences are a small
  hand-written template set (~30 sentences, ~100 distinct words), not a
  real corpus — outputs demonstrate the Gumbel-Softmax + straight-through
  training technique, not fluent open-domain text. Swapping in a real
  dataset (e.g. a sentence corpus + BPE tokenizer) and a larger model
  would be the natural next step for actual text quality.
...
- The `API_KEY` gate (see "Auth") is a shared secret, not per-user auth —
  fine for "keep random visitors from triggering training on my demo,"
  not a substitute for real authentication if this were a multi-user app.
